...
mp.events.add("server:vehKeys:openKeyCopy", (player) => {
    gm.mysql.handle.query("SELECT vehiclekeys.*, vehicles.numberPlate, vehicles.model from vehiclekeys LEFT JOIN vehicles ON vehiclekeys.vehID = vehicles.id WHERE vehiclekeys.keyOwner = ? AND vehiclekeys.isActive='Y'", [player.data.charId], function (err, res) {
        if (err) console.log(err);
        if (res.length > 0) {
            var i = 1;
            let KeyList = [];
            ...
            if (res.length > 0) {             
                ...
                if ( (amount*1) > (money*1)) {
                    ...
                    gm.mysql.handle.query("UPDATE characters SET money = ? WHERE id = ?", [newMoney, player.data.charId], function (err3, res3){
                        ...
                        gm.mysql.handle.query("SELECT vehiclekeys.*, vehicles.* from vehiclekeys LEFT JOIN vehicles ON vehiclekeys.vehID = vehicles.id WHERE vehiclekeys.keyOwner = ? AND vehiclekeys.isActive='Y'", [player.data.charId], function(err1, res1){
                            ...
                            if (res1.length > 0) {               
                                res1.forEach(function(keys) {
                                    ...
                                    gm.mysql.handle.query("UPDATE vehiclekeys SET amount = ? WHERE id = ? AND keyOwner = ?", [newAm, selectedDescription, player.data.charId], function (err2, res2) {
                                        ...
                                        player.notify("~g~Du hast den Schlüssel nachgemachst!");
                                    });                                    
                                });                                
                            } else {
                                player.notify("~r~Diesen Schlüssel besitzt du nicht!");
                            }
                        });
                    });
                }
...
mp.events.add("server:vehKeys:schloss", (player, vehicle) => {
    if(mp.players.exists(player)) {
        let numberplate = vehicle.getVariable("vehId");
        gm.mysql.handle.query("SELECT * FROM vehicles WHERE id = ?", [numberplate], function(err1, res1) {
                    ...
                    if (err2) console.log("Error in Update vehiclekeys: "+err2);
                        gm.mysql.handle.query("INSERT INTO vehiclekeys(vehID,keyOwner,amount,isActive) VALUES(?,?,'2','Y')", [veh.id, player.data.charId], function(err3, res3) {
                            if(err3) console.log("Error in Insert new vehiclekeys: "+err3);
                                ...
                                gm.mysql.handle.query("SELECT c.isOnline, c.onlineId FROM vehiclekeys v LEFT JOIN characters c ON c.id = v.keyOwner WHERE v.vehID = ?", [veh.id], function(err4, res4) {
