...
mp.events.add("server:carshop:buy",(player, id, name) => {
    gm.mysql.handle.query("SELECT * FROM carshop_cars WHERE id = ?", [id], function (err,res) {
        ...
        if (res.length > 0) {
            gm.mysql.handle.query("SELECT * FROM bank_konten WHERE ownerId = ? AND firma = '0'",[player.data.charId], function(err1,res1) {
                             ...
                             const onehead = res2[0].spawnR; 
                                if (getVehicleFromPosition(one, 3).length > 0) {
                                    ...
                                    gm.mysql.handle.query("UPDATE bank_konten SET amount = ? WHERE ownerId = ? AND firma = '0'",[newAm,player.data.charId],function(err3,res3) {
                                        ...
                                        gm.mysql.handle.query("INSERT INTO vehicles SET model = ?, charId = ?, numberPlate = 'MR', parked = '1', impounded = '0', fill = '100', km = '0', fuelart = ?, kofferraum = ?, dead = 'false'",[name, player.data.charId,res[0].tankart,res[0].kofferraum], function (err4,res4) {
                                    ...
                                    gm.mysql.handle.query("UPDATE bank_konten SET amount = ? WHERE ownerId = ? AND firma = '0'",[newAm,player.data.charId],function(err3,res3) {
                                        if (err3) console.log("Error in Update bank: "+err3);
                                        player.data.bank = newAm;
                                        gm.mysql.handle.query("INSERT INTO vehicles SET model = ?, charId = ?, numberPlate = 'MR', parked = '0', impounded = '0', fill = '100', km = '0', fuelart = ?, kofferraum = ?, dead = 'false'",[name, player.data.charId,res[0].tankart,res[0].kofferraum], function (err4,res4) {
                                            if (err4) console.log("Error in Insert Vehicles: "+err4);
