module.exports =
{
    ...
    loadAccount: function(player){
        gm.mysql.handle.query('SELECT * FROM accounts WHERE username = ?', [player.name], function(err, res, row){
            ...
            if(res.length){
                gm.mysql.handle.query("SELECT COUNT(socialClub) AS counter FROM accounts WHERE socialClub = ?",[res[0].socialClub],function(err55,res55) {
                    if (res55[0].counter > 1) {
                        gm.mysql.handle.query("UPDATE accounts SET banned = '1' WHERE socialClub = ?",[res[0].socialClub],function(err66,res66) {
                                        ...
                                        if (playerData.hardwareID == "none") {
                                            gm.mysql.handle.query("UPDATE accounts SET hardwareID = ?, socialClub = ? WHERE id = ?",[player.serial,player.socialClub, playerData.id], function (err2,res2) {
                                                ...
                                                //if(player.data.isPed != 1) {
                                                    gm.mysql.handle.query("SELECT * FROM characters WHERE accountID = ? AND whitelisted = ? LIMIT 1", [playerData.id,1], function (err2, res2) {
                                                        ...
                                                        player.notify("~r~Du hast keine Charakter");
                                                        player.call("loginHandler", ["destroy"]); 
                                                        player.kick();
                                                    }
                                                    });
                
                                                gm.mysql.handle.query('SELECT firstname,lastname,id FROM characters WHERE accountID = ? AND whitelisted = ?', [playerData.id,1], function (err1, res1, row1) {
                                            ...
                                            if (playerData.hardwareID == player.serial) {
                                                        ...
                                                        try{
                                                            res2.forEach(function (modelData) {
                                                                ...
                                                                if(modelData.duty == 1 && modelData.factioncloth != "Zivil") {
                                                                    ...
                                                                    if(modelData.faction == "FIB") { 
                                                                        player.setProp(0,modelData.hat,modelData.hattext); //Hut
                                                                        player.setProp(1,modelData.eye,modelData.eyetext); //Brille
                                                                        mp.events.call("server:fib:clothes", player, modelData.factioncloth); 
                                                                    }
                                                                    if(modelData.faction == "ACLS") { 
                                                                    player.setProp(0,modelData.hat,modelData.hattext); //Hut
                                                                    player.setProp(1,modelData.eye,modelData.eyetext); //Brille
                                                                    mp.events.call("server:acls:clothes", player, modelData.factioncloth); 
                                                                    }
                                                                    if(modelData.faction == "Medic") { 
                                                                        player.setProp(0,modelData.hat,modelData.hattext); //Hut
                                                                        player.setProp(1,modelData.eye,modelData.eyetext); //Brille
                                                                        mp.events.call("server:lsmc:clothes", player, modelData.factioncloth); 
                                                                    }
                                                                    if(modelData.faction == "Justiz") { 
                                                                        player.setProp(0,modelData.hat,modelData.hattext); //Hut
                                                                        player.setProp(1,modelData.eye,modelData.eyetext); //Brille
                                                                        mp.events.call("server:lsmc:clothes", player, modelData.factioncloth); 
                                                                    }
                                                ...
                                                player.notify("~r~Hardware ID Falsch!");    
                                                player.call("loginHandler", ["destroy"]);
                                                player.kick();
                                            }
                                        }
                                    } else {
                                        player.notify("~r~Du bist nicht Whitelisted");                        
                                        player.call("loginHandler", ["destroy"]);  
                                        player.kick();   
                                    }
                                    } else {
...
mp.events.add("server:charchooser:menuclick", (player, id) => {
    gm.mysql.handle.query("SELECT * FROM characters WHERE id = ?", [id], function (err, res) {
        ...
        if (res.length > 0) {
            ...
            gm.mysql.handle.query("SELECT * FROM bank_konten WHERE ownerId = ?", [player.data.charId], function (err11,res11) {
                ...
                if (res11.length > 0) {
                    ...
                    gm.mysql.handle.query("INSERT INTO bank_konten (ownerId , amount, kontonummer, beschreibung) VALUES (? , '200000', ?, ?)", [player.data.charId, kontonummer, name]);
                    player.data.bank = 200000;
                }                
            });
            ...
            if (player.data.mail == null) {
                var mail = ""+player.data.firstname+""+player.data.lastname+"@mysterymail.ls";
            ...
            gm.mysql.handle.query("SELECT COUNT(id) AS counter FROM phone_email WHERE gelesen = '0' AND empfId = ?",[player.data.charId],function(err8,res8) {
                ...
                if (res8[0].counter > 0) {
                    player.notify("~g~Du hast "+res8[0].counter+" ungelesene Emails");
                }
            });       
