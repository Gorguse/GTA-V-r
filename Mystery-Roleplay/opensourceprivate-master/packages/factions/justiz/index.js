let duty = mp.colshapes.newSphere(119,-731,242, 1, 0);
let chief = mp.colshapes.newSphere(137,-760,258, 1, 0);
let pc = mp.colshapes.newSphere(126,-767,242, 1, 0);
...
mp.events.add("server:justiz:mainMenu", (player,slot,name) => {
	...
	if(name == "Dienstausweis zeigen")
	{
        ...
        if (currentTarget !== null) {
            ...
            gm.mysql.handle.query("SELECT * FROM payChecks WHERE factionrang = ? AND faction = ?",[player.data.factionrang,player.data.faction], function(err,res) {
                ...
                currentTarget.call("client:dokumente:showjustiz",[player.data.firstname,player.data.lastname,res[0].rankname]);
                setTimeout(_ => {
    ...
    {
        player.call("client:justiz:openMemberMenu");
    }
});
mp.events.add("server:justiz:memberMenu", (player,slot) => {
    getNearestPlayer(player, 2);    
    if(slot == 0)
    {
        if(currentTarget.data.faction == "Civillian")
        {
           // Person Einstellen     
            ...
            player.notifyWithPicture("Department of Justice", "Einstellung", "Hat bereits eine Anstellung!","CHAR_CALL911");
        }
    }
    else if (slot == 1)
    {
    ...
    else if(slot == 3)
    {
        if(currentTarget.data.faction == "Justiz")
        {
            ...
            player.call("client:justiz:closeMemberMenu");
            player.call("client:justiz:askedForDismiss");
        }
    }
});
...
mp.events.add("server:justiz:mitarbeiter",(player) => {
    gm.mysql.handle.query("SELECT firstname,lastname,factionrang,id FROM characters WHERE faction = 'Justiz'",[],function(err,res) {
        ...
        if (res.length > 0) {
            ...
            player.notify("Es gibt keine Justiz Beamten");
        }
     });
});
