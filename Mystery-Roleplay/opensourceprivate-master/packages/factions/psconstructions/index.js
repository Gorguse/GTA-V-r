let pc = mp.colshapes.newSphere(122,6406,32, 1, 0);
let vehicle = mp.colshapes.newSphere(179,6392,32, 1, 0);
...
mp.events.add("server:ps:mainMenu", (player,slot,name) => {
    getNearestPlayer(player, 2);   
    if (name == "Mitarbeiterverwaltung")
    {
        player.call("client:ps:openMemberMenu");
    } 
});
mp.events.add("server:ps:entlassen", (player,itemText) => {
    getNearestPlayer(player, 2);   
    if(currentTarget !== null) {    
        if (currentTarget.data.faction == "PSConstructions") {
            currentTarget.notifyWithPicture("PS Constructions", "Entlassung", "Deine Anstellung wurde beendet","CHAR_CALL911");
        currentTarget.call("client:ps:delmarkers");
        player.notifyWithPicture("PS Constructions", "Mitarbeiterverwaltung","Du hast "+currentTarget.data.firstname + " "+ currentTarget.data.lastname + " entlassen.","CHAR_CALL911");
        ...
        gm.mysql.handle.query("SELECT * FROM characters WHERE id = ?",[currentTarget.data.charId], function(err5,res5) {
            ...
            player.notify("~r~Der Spieler Arbteitet nicht bei der PS Constructions!")
        } 
    }   
});

mp.events.add("server:ps:befördern", (player,rank) => {
    if (mp.players.exists(player)) {
        getNearestPlayer(player,2);
        if (currentTarget !== null) {
            if (currentTarget.data.faction == "PSConstructions") {
                ...
                player.notify("~r~Die Person ist nicht bei PS Constructions");
            }
        } else {
...
mp.events.add("server:ps:mitarbeiter",(player) => {
    gm.mysql.handle.query("SELECT firstname,lastname,factionrang,id FROM characters WHERE faction = 'PSConstructions'",[],function(err,res) {
        ...
        if (res.length > 0) {
            ...
            res.forEach(function(justiz) {
                ...
                if (parseInt(i) == parseInt(res.length)) {
                    if (mp.players.exists(player)) player.call("client:ps:Memberlist", [JSON.stringify(LSCList)]);
                }
                i++;
            });
        } else {   
            player.notify("Es gibt keine PSConstructions Mitarbeiter");
        }
     });
});

mp.events.add("server:ps:mitarbeiterentl",(player,id) => {
...
mp.events.add("server:ps:einstellen",(player) => {
    getNearestPlayer(player, 2);  
    if (currentTarget !== null) {
        if (currentTarget.data.faction == "Civillian") {
            gm.mysql.handle.query("UPDATE characters SET faction = 'PSConstructions', factionrang = '1',firma = '1' WHERE id = ?",[currentTarget.data.charId], function (err,res) {
                if (err) console.log("Error in Update Faction user: "+err);
                player.notify("~g~Der Bürger wurde eingestellt");
                currentTarget.notify("Sie wurden bei PSConstructions eingestellt!");
                currentTarget.data.faction = "PSConstructions";
                currentTarget.data.factionDuty = 0;
...
mp.events.add("server:ps:spawnVehicle",(player,type) => {
    const one = new mp.Vector3(183.9088897705078, 6394.70263671875, 31.3815975189209);
    const onehead = 296;
    const two = new mp.Vector3(181.94921875, 6402.14111328125, 31.301345825195312);
    const twohead = 296;   
    if (getVehicleFromPosition(one, 3).length > 0) {    
        if (getVehicleFromPosition(two, 3).length > 0) {   
            ...
            veh.setVariable("faction", "PSConstructions");
...
mp.events.add("server:ps:parkin",(player,x,y,z) => {
