let impound = mp.colshapes.newSphere(-350.6612854003906, -117.08207702636719, 38.927921295166016, 5, 0);
...
mp.events.add("server:lscgarage:openMenu",(player) => {
    gm.mysql.handle.query("SELECT v.*, c.firstname, c.lastname FROM vehicles v LEFT JOIN characters c ON c.id = v.charId WHERE v.impounded = '2'",[],function(err,res) {
...
mp.events.add("server:lscgarage:parkin", (player,id) => {
        ...
        gm.mysql.handle.query("UPDATE vehicles SET parked = '1', impounded = '2' WHERE id = ?",[veh.getVariable("vehId")], function(err2,res2) {
