let impound = mp.colshapes.newSphere(-234.15396118164062, -1697.9088134765625, 33.909141540527344, 5, 0);
mp.events.add("PushE", (player) => {
  if (mp.players.exists(player)) {
    if(impound.isPointWithin(player.position)) {
        if (player.data.gang == "Ballas") {
           player.call("client:ballas:openMainMenu");
...
mp.events.add("server:ballas:openMenu",(player) => {
    gm.mysql.handle.query("SELECT v.*, c.firstname, c.lastname FROM vehicles v LEFT JOIN characters c ON c.id = v.charId WHERE v.impounded = '6'",[],function(err,res) {
        if (err) console.log("Error in Select Ballas Vehicles: "+err);
        if (res.length > 0) {
            var i = 1;
            let vehList = [];
            res.forEach(function(veh) {
                ...
                if (parseInt(i) == parseInt(res.length)) {
                    if(mp.players.exists(player)) player.call("client:ballas:openMenu", [JSON.stringify(vehList)]);
                }
                i++;
            });
        } else {
            player.notify("~r~Keine Fahrzeuge in der Ballas Garage");
        }
    });
});


mp.events.add("server:ballas:parkout",(player,id) => {
    const one = new mp.Vector3(-218.80740356445312, -1703.368408203125, 34.00327682495117);
    const onehead = 39;
    gm.mysql.handle.query("SELECT * FROM vehicles WHERE id = ?",[id],function(err1,res1) {
        if (err1) console.log("Error in Select Vehicles on ballas: "+err1);
...
mp.events.add("server:ballas:parkin", (player,id) => {
        const pos = new mp.Vector3(-228.11756896972656, -1692.588134765625, 33.78940963745117);
        ...
        gm.mysql.handle.query("UPDATE vehicles SET parked = '1', impounded = '6' WHERE id = ?",[veh.getVariable("vehId")], function(err2,res2) {
